    ...
    useEffect(() => {
        const getCurrentVideo = () => {
            ...
            if (!courseId && !sectionId && !subSectionId) {
                ...
                const filteredData = courseSectionData?.find((section) => section._id === sectionId);

                // Check if filteredData and filteredVideoData exist before proceeding
                if (filteredData) {
                    const filteredVideoData = filteredData?.subSection?.find((subsection) => subsection._id === subSectionId);
                    if (filteredVideoData) {
                        setVideoData(filteredVideoData);
                        setPreviewSource(courseEntireData.thumbnail);
                    } else {
                        console.log("No video found for the given sub-section.");
                    }
                } else {
                    console.log("No section found for the given section ID.");
                }
            }
        }
        getCurrentVideo();
                                ...
                                <div className="mt-10 flex min-w-[250px] justify-center gap-x-4 text-xl">
                                    {!isFirstVideo() && (
                                        <button disabled={loading}
                                            onClick={goToPreviousVideo} className="blackButton">Prev</button>
                                    )}
                                    {!isLastVideo() && (
